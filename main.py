...
@client.event
async def on_ready():
    await client.change_presence(status=discord.Status.online, activity=discord.Game("HolyFrag.pl"))
...
class rangiPrzyciski(discord.ui.View):
  def __init(self):
    super().__init__(timeout=None)
    self.value = None

  @discord.ui.button(label="Silver", style=discord.ButtonStyle.grey)
  async def aaarole(self, button: discord.ui.Button, interaction: discord.Interaction):
    silv = discord.utils.get(interaction.user.guild.roles, id=919624212581330964)
    await interaction.user.add_roles(silv)
    await interaction.response.send_message(":white_check_mark: Pomyślnie Nadano :white_check_mark:", ephemeral=True)
    self.value = True

  @discord.ui.button(label="Gold", style=discord.ButtonStyle.grey)
  async def baarole(self, button: discord.ui.Button, interaction: discord.Interaction):
    gold = discord.utils.get(interaction.user.guild.roles, id=919624212170285056)
    await interaction.user.add_roles(gold)
    await interaction.response.send_message(":white_check_mark: Pomyślnie Nadano :white_check_mark:", ephemeral=True)
    self.value = True

  @discord.ui.button(label="Kałach", style=discord.ButtonStyle.grey)
  async def caaarole(self, button: discord.ui.Button, interaction: discord.Interaction):
    kala = discord.utils.get(interaction.user.guild.roles, id=919624172303446057)
    await interaction.user.add_roles(kala)
    await interaction.response.send_message(":white_check_mark: Pomyślnie Nadano :white_check_mark:", ephemeral=True)
    self.value = True

  @discord.ui.button(label="Szeryf", style=discord.ButtonStyle.grey)
  async def daaarole(self, button: discord.ui.Button, interaction: discord.Interaction):
    szery = discord.utils.get(interaction.user.guild.roles, id=919624173804998676)
    await interaction.user.add_roles(szery)
    await interaction.response.send_message(":white_check_mark: Pomyślnie Nadano :white_check_mark:", ephemeral=True)
    self.value = True

  @discord.ui.button(label="Orzełek", style=discord.ButtonStyle.grey)
  async def eaaarole(self, button: discord.ui.Button, interaction: discord.Interaction):
    orze = discord.utils.get(interaction.user.guild.roles, id=919623020857925632)
    await interaction.user.add_roles(orze)
    await interaction.response.send_message(":white_check_mark: Pomyślnie Nadano :white_check_mark:", ephemeral=True)
    self.value = True

  @discord.ui.button(label="Legendarny Orzełek", style=discord.ButtonStyle.grey)
  async def raaarole(self, button: discord.ui.Button, interaction: discord.Interaction):
    lege = discord.utils.get(interaction.user.guild.roles, id=919624171166769173)
    await interaction.user.add_roles(lege)
    await interaction.response.send_message(":white_check_mark: Pomyślnie Nadano :white_check_mark:", ephemeral=True)
    self.value = True

  @discord.ui.button(label="Supreme", style=discord.ButtonStyle.grey)
  async def taaarole(self, button: discord.ui.Button, interaction: discord.Interaction):
    supr = discord.utils.get(interaction.user.guild.roles, id=919624170730565632)
    await interaction.user.add_roles(supr)
    await interaction.response.send_message(":white_check_mark: Pomyślnie Nadano :white_check_mark:", ephemeral=True)
    self.value = True

  @discord.ui.button(label="Global Elite", style=discord.ButtonStyle.grey)
  async def yaaarole(self, button: discord.ui.Button, interaction: discord.Interaction):
    glob = discord.utils.get(interaction.user.guild.roles, id=919624170000752640)
    await interaction.user.add_roles(glob)
    await interaction.response.send_message(":white_check_mark: Pomyślnie Nadano :white_check_mark:", ephemeral=True)
    self.value = True
...
async def ran(ctx):
  ...
  view = rangiPrzyciski()
  embed=discord.Embed(color=0xff0000)
  embed.add_field(name="Klikając przycisk, możesz wybrać swoją rangę", value="--------------------------------------------------------------", inline=True)
  embed.set_footer(text="HolyFrag.pl © 2022")
  await ctx.send(embed=embed, view=view)
  await view.wait()
